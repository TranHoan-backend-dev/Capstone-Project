...
public class SettlementController {
    ...
    public ResponseEntity<WrapperApiResponse> createSettlement(@RequestBody @Valid SettlementRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(new WrapperApiResponse(
            HttpStatus.CREATED.value(), "Tạo quyết toán công trình thành công", response, LocalDateTime.now()));
    }
    ...
    public ResponseEntity<WrapperApiResponse> updateSettlement(
        ...
        return ResponseEntity.ok(new WrapperApiResponse(
            HttpStatus.OK.value(), "Cập nhật quyết toán công trình thành công", response, LocalDateTime.now()));
    }
    ...
    public ResponseEntity<WrapperApiResponse> deleteSettlement(
        ...
        return ResponseEntity.ok(new WrapperApiResponse(
            HttpStatus.OK.value(), "Xóa quyết toán công trình thành công", null, LocalDateTime.now()));
    }
    ...
    public ResponseEntity<WrapperApiResponse> getSettlementById(
        ...
        return ResponseEntity.ok(new WrapperApiResponse(
            HttpStatus.OK.value(), "Lấy thông tin quyết toán công trình thành công", response, LocalDateTime.now()));
    }
    ...
    public ResponseEntity<WrapperApiResponse> getAllSettlements(
        ...
        return ResponseEntity.ok(new WrapperApiResponse(
            HttpStatus.OK.value(), "Lấy danh sách quyết toán công trình thành công", response, LocalDateTime.now()));
    }

    @GetMapping("/filter")
    @Operation(summary = "Filter settlements with search and criteria", description = "Returns a paginated list of settlements filtered by search term, status, date ranges, and connection fee range.", responses = {
        @ApiResponse(responseCode = "200", description = "Filtered list of settlements retrieved successfully")
    })
    public ResponseEntity<WrapperApiResponse> filterSettlements(
        @ModelAttribute @Parameter(description = "Filter parameters") SettlementFilterRequest filterRequest,
        @PageableDefault @Parameter(description = "Pagination parameters") Pageable pageable) {
