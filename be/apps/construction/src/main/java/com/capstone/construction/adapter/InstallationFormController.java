...
public class InstallationFormController {
    ...
    @Operation(summary = "Kiểm tra tồn tại của đơn lắp đặt", description = "API này kiểm tra xem một đơn lắp đặt có tồn tại không dựa trên mã biểu mẫu và số biểu mẫu", responses = {
        @ApiResponse(responseCode = "200", description = "Trả về true nếu tồn tại, false nếu không tồn tại"),
        @ApiResponse(responseCode = "400", description = "Lỗi dữ liệu đầu vào")
    })
    @GetMapping("/exist")
    @PreAuthorize("hasAnyAuthority('ORDER_RECEIVING_STAFF', 'IT_STAFF')")
    public ResponseEntity<WrapperApiResponse> checkInstallationFormExistence(
        @RequestParam String formCode,
        @RequestParam String formNumber
    ) {
        log.info("Checking existence of installation form with code: {} and number: {}", formCode, formNumber);
        boolean exists = installationFormService.isInstallationFormExisting(formNumber, formCode);
        return Utils.returnOkResponse("Kiểm tra tồn tại thành công", exists);
    }
